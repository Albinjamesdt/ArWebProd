    ...
    try {
      ...
      const constraints = {
        video: {
          ...
          width: isMobile ? { ideal: 720, min: 480 } : { ideal: 1280, min: 640 },
      ...
      <div className="min-h-screen flex items-center justify-center bg-gray-900 p-4">
        <Card className="w-full max-w-md bg-gray-800 border-gray-700">
          <CardContent className="flex flex-col items-center justify-center p-6">
            ...
            <h3 className="text-white font-semibold mb-3 text-xl text-center">No Content Available</h3>
        ...
        <div className={`flex justify-between items-start gap-${isMobile ? "2" : "4"}`}>
          <Card className="bg-black/80 border-gray-700 flex-1">
            <CardContent className={`p-${isMobile ? "2" : "3"}`}>
              <div className={`flex items-center gap-2 text-${isMobile ? "xs" : "sm"} text-white`}>
                <Camera className={`w-${isMobile ? "3" : "4"} h-${isMobile ? "3" : "4"}`} />
                <span>WebAR</span>
                {cameraReady && arInitialized && (
          ...
          <Card className="bg-black/80 border-gray-700">
            <CardContent className={`p-${isMobile ? "2" : "3"}`}>
              <div className={`text-${isMobile ? "xs" : "sm"} text-white text-right`}>
                <div className="flex items-center gap-1 justify-end mb-1">
                  <Scan className={`w-${isMobile ? "3" : "4"} h-${isMobile ? "3" : "4"}`} />
                  <span>{markers.length}</span>
                </div>
                <div className="text-xs text-gray-400">
                  {currentMarkerInfo ? (isMobile ? "Playing" : `Playing: ${currentMarkerInfo.title}`) : "Scanning..."}
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card className="bg-black/80 border-gray-700">
            <CardContent className={`p-${isMobile ? "3" : "4"} text-center`}>
              <div className="flex items-center justify-center gap-2 mb-2">
                ...
                />
                <span className={`text-white font-medium text-${isMobile ? "sm" : "base"}`}>
        ...
        <div className="absolute inset-0 flex items-center justify-center z-10 pointer-events-none px-4">
          <div className={`${isMobile ? "w-64 h-40" : "w-72 h-48"} border-2 border-white/40 rounded-xl relative`}>
            ...
            <div className="absolute inset-0 flex items-center justify-center">
              <span
                className={`text-white text-${isMobile ? "xs" : "sm"} bg-black/60 px-3 py-1 rounded-full text-center`}
              >
